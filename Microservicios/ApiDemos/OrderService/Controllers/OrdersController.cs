    ...
    EndpointUrls urls) : ControllerBase
{
    ...
    public async Task<ActionResult<OrderResponseDto>> Create([FromBody] OrderRequestDto req)
    {
        ...
        // 3) Registrar evento en Outbox (patrón outbox simplificado)
        await db.Outbox.AddAsync(new OutboxEventEntity
        {
            OrderId = order.Id,
            PayloadJson = "{}" // opcional: puedes serializar el DTO
        });
